...
3. Append a line to `.agents/checkpoints.log` (create if missing):
...
Read `.agents/checkpoints.log` and print as a table (date, label, identifier). If the log doesn't exist, say so.

### `restore <label>`

1. Look up `<label>` in `.agents/checkpoints.log`. Refuse if ambiguous (multiple matches); ask for the timestamp.
...
- Don't write to `.agents/checkpoints.log` without recording enough to find the checkpoint later — label alone isn't enough; capture the stash ref or sha.

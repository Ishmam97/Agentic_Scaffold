...
The MCP server (`obsidian`) is wired in `.codex/config.toml` for Codex or `.mcp.json` for Claude Code and exposes the tools below. Reach for the vault when the user talks about notes/memory/knowledge; reach for `docs/` when producing a planning artifact.

...
**Deferred-corner check.** Grep the diff for `# scope:` markers (see CLAUDE.md § Conventions). Any left in the diff are corners the author flagged as intentionally cut — list each with file:line so the author can confirm it's still acceptable to ship, or convert it to a follow-up story/ADR.
